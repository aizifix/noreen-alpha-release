...
export default function VenuesPage() {
  ...
  useEffect(() => {
    ...
    if (filters.priceRange) {
      ...
      filtered = filtered.filter((venue) => {
        const price = venue.extra_pax_rate;
        if (max) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await axios.post(endpoints.admin, {
        ...
        extra_pax_rate: editForm.extra_pax_rate,
      });
  ...
  const getStats = () => {
    ...
    const avgPrice =
      venues.length > 0
        ? Math.round(
            venues.reduce((sum, v) => sum + v.extra_pax_rate, 0) / venues.length
          )
        : 0;
    const maxCapacity =
                        ...
                        <div className="space-y-3 mb-6">
                          ...
                          <div className="flex items-center justify-between text-sm">
                            <div className="flex items-center text-gray-600">
                              ...
                              <span>Extra Pax Rate</span>
                            </div>
                            <span className="font-medium text-gray-900">
                              ₱{venue.extra_pax_rate.toLocaleString()}
                            </span>
                          </div>
                          {venue.venue_type && (
                            <div className="flex items-center justify-between text-sm">
