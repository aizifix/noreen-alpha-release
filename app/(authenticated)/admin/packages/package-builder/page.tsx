...
import { VenueFeeInclusion } from "./venue-fee-inclusion";
...
export default function PackageBuilderPage() {
  ...
  const [totalPackagePrice, setTotalPackagePrice] = useState<number | null>(
    null
  ); // Admin's total package price
  ...
  // Inclusions state - Initialize with one empty inclusion (venue fees handled separately)
  ...
  // Note: Venue fee buffer is now handled separately in VenueFeeInclusion component
  ...
  const validatePackagePricing = () => {
    ...
    // Note: Venue fee validation is now handled separately in VenueFeeInclusion component
  ...
  const areInclusionsValid = () => {
    return (
      ...
      inclusions.some((inclusion) => {
        // Exclude venue_fee from regular inclusion validation
      ...
      <div className="space-y-6">
            ...
            <div>
              ...
              {totalPackagePrice !== null && totalPackagePrice > 0 && (
                <p className="text-xs text-green-600 mt-1">
                  Formatted: ₱
                  {totalPackagePrice.toLocaleString("en-US", {
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Maximum amount allocated for venue costs. If selected venue
              exceeds this, client pays the difference.
            </p>
            {venueFeeBuffer !== null && venueFeeBuffer > 0 && (
              <p className="text-xs text-green-600 mt-1">
                Formatted: ₱
                {venueFeeBuffer.toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </p>
              ...
              {eventTypes &&
                eventTypes.map((type) => {
                  const getEventIcon = (eventName: string) => {
                    ...
                    if (name.includes("corporate") || name.includes("business"))
                      return <Calendar className="w-5 h-5" />;
                    if (name.includes("party") || name.includes("celebration"))
                      return <Music className="w-5 h-5" />;
                    if (name.includes("photo") || name.includes("shoot"))
        ...
        {venueOverages.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            ...
            <p className="text-sm text-amber-700 mb-3">
              The following venues exceed your venue fee buffer of ₱
            ...
            <p className="text-xs text-amber-600 mt-2">
              Clients will need to pay the excess amount when booking these
              venues.
            </p>
          </div>
        )}
  ...
  const ReviewStepElement = useMemo(() => {
    const selectedEventTypeName =
      selectedEventType === null
        ? "Not selected"
        : eventTypes.find((et) => et.event_type_id === selectedEventType)
            ?.event_name || "Unknown";
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{selectedEventTypeName}</p>
              </div>
            </div>
          </div>
  ...
  const PackageSummary: React.FC = () => {
    ...
    const selectedEventTypeName = useMemo(() => {
      ...
      const eventType = eventTypes.find(
        (et) => et.event_type_id === selectedEventType
      );
            ...
            <div className="pt-3 border-t">
              ...
              <div className="space-y-2 max-h-32 overflow-auto pr-1">
                {(inclusions || [])
                  .filter(
                    (inc) =>
                      inc.name.trim() !== "" && inc.category !== "venue_fee"
                  )
                  .map((inc, i) => (
                ...
                {(!inclusions ||
                  inclusions.every((inc) => !inc.name.trim())) && (
  ...
  const steps: Step[] = [
    ...
    {
      id: "venue-fees",
      title: "Venue Fee Management",
      description: "Set venue fee buffer and manage venue costs",
      component: (
        <div className="grid lg:grid-cols-3 gap-6">
    ...
    },
    {
      ...
      component: (
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">{ReviewStepElement}</div>
          <div>
            <PackageSummary />
          </div>
        </div>
