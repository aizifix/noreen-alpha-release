...
import { Calendar, Clock, User, DollarSign, Package, MapPin, Mail, Activity } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  const fetchTimeline = async () => {
    ...
    try {
      const response = await fetch(`http://localhost/events-api/admin.php?operation=getActivityTimeline&admin_id=${adminId}&start_date=${startDate}&end_date=${endDate}&limit=100`);
      ...
      setError('Error fetching timeline data: ' + (err instanceof Error ? err.message : 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex flex-col space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">System Activity Timeline</h1>
            <p className="text-gray-600 mt-2">Track all activities and actions taken within the system</p>
          </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="flex items-end">
                <Button 
                  onClick={() => {
                    setStartDate(format(new Date().setDate(1), 'yyyy-MM-dd'));
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {timeline.filter(item => item.action_type === 'event_created').length}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {timeline.filter(item => item.action_type.includes('payment')).length}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {timeline.filter(item => ['organizer_activity', 'supplier_activity'].includes(item.action_type)).length}
                  </p>
                </div>
              </div>
            </CardContent>
              ...
              <div className="space-y-4">
                {filteredTimeline.map((item, index) => (
                  <div key={index} className="flex items-start space-x-4 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                    <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${getActionColor(item.action_type)}`}>
                      {getActionIcon(item.action_type)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-1">
                        <p className="text-sm font-medium text-gray-900">{item.description}</p>
                        <Badge className={`text-xs ${getUserTypeColor(item.user_type)}`}>
                          {item.user_type}
                        </Badge>
                      </div>
