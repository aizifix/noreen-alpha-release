...
const documentTypes = [
  { code: 'dti', name: 'DTI Permit', required: true, description: 'Department of Trade and Industry business registration permit' },
  { code: 'business_permit', name: 'Business Permit', required: true, description: 'Local government business permit and licenses' },
  { code: 'contract', name: 'Service Contract', required: true, description: 'Signed service agreements and contracts' },
  { code: 'portfolio', name: 'Portfolio', required: false, description: 'Work samples and portfolio documents' },
  { code: 'certification', name: 'Certification', required: false, description: 'Professional certifications and awards' },
  { code: 'other', name: 'Other Documents', required: false, description: 'Miscellaneous supporting documents' }
];
...
export default function EnhancedSupplierCreationForm() {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0 || !currentDocumentType || !currentDocumentTitle.trim()) return;
    ...
    if (file.size > maxSize) {
      setErrors(prev => ({ ...prev, documents: 'File size exceeds 10MB limit' }));
      return;
    }

    if (!allowedTypes.includes(file.type)) {
      setErrors(prev => ({ ...prev, documents: 'Invalid file type. Only PDF, JPG, and PNG files are allowed' }));
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin.php?operation=createSupplier', {
        method: 'POST',
        body: formDataToSend
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-gray-50 p-6 rounded-lg space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="contact_number" className="flex items-center space-x-2">
          ...
          <div className="bg-gray-50 p-6 rounded-lg space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="supplier_type" className="flex items-center space-x-2">
                  <span>Supplier Type</span>
                  <span className="text-red-500">*</span>
                </Label>
                <Select
                  value={formData.supplier_type}
                  onValueChange={(value: 'internal' | 'external') => handleInputChange('supplier_type', value)}
                >
                  <SelectTrigger className={errors.supplier_type ? 'border-red-500' : ''}>
                    <SelectValue placeholder="Select supplier type" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="internal">Internal Supplier (With Portal Access)</SelectItem>
                    <SelectItem value="external">External Supplier (No Portal Access)</SelectItem>
                  </SelectContent>
                </Select>
                {errors.supplier_type && <p className="text-red-500 text-sm mt-1">{errors.supplier_type}</p>}
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => handleInputChange('specialty_category', value)}
                >
                  <SelectTrigger>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleInputChange('agreement_signed', checked)}
                />
          ...
          {formData.supplier_type === 'internal' && (
            <div className="bg-blue-50 p-6 rounded-lg space-y-4">
              ...
              <Alert className="bg-blue-100 border-blue-300 text-blue-800">
                <Mail className="w-4 h-4" />
                <AlertDescription>
                  The generated password will be automatically sent to the supplier's email address upon account creation.
                  The supplier will be required to change this password on first login.
                </AlertDescription>
              </Alert>
            </div>
          )}
          ...
          <div className="bg-gray-50 p-6 rounded-lg space-y-4">
            <h3 className="text-lg font-semibold text-gray-900 flex items-center">
              ...
              Documents Upload
              {formData.supplier_type === 'internal' && (
                <Badge variant="secondary" className="ml-2">Required for Internal Suppliers</Badge>
              )}
            </h3>
                  ...
                  <SelectContent>
                    {documentTypes.map(type => (
                      <SelectItem key={type.code} value={type.code}>
                        {type.name} {type.required && formData.supplier_type === 'internal' && <span className="text-red-500">*</span>}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Button
                    ...
                    disabled={!currentDocumentType || !currentDocumentTitle.trim()}
                  >
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {documentTypes.find(type => type.code === doc.type)?.name} • {(doc.file.size / 1024 / 1024).toFixed(2)} MB
                          </p>
                        </div>
                      </div>
                      <Button
